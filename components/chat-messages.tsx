...
export function ChatMessages({ messages, isLoading, user }: ChatMessagesProps) {
  ...
  return (
    <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8 flex-1 overflow-y-auto">
      ...
      {messages.map((message) => {
        if (message.contentType === 'image' && message.content) {
          console.log("Image message - content length:", message.content.length, "starts with:", message.content.substring(0, 50));
          if (message.content.startsWith('data:image')) {
            console.log("Image content is valid base64 data URI");
          } else if (message.content.startsWith('blob:')) {
            console.log("Image content is blob URL (local preview)");
          } else {
            console.log("Image content is neither base64 nor blob - may not render properly");
          }
        }
        return (
          <motion.div
          ...
          >
            <div className={`flex items-start space-x-3 max-w-3xl ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
              ...
              <div className={`rounded-2xl px-5 py-4 ${
                ...
                {message.type === 'user' && message.transcription && (
